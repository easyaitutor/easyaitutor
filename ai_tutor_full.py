...
def send_email_notification(to_email, subject, html_content,
                            from_name="AI Tutor System",
                            attachment_filepath_str=None,
    ...
    if attachment_filepath_str:
        try:
            file_data = open(attachment_filepath_str,'rb').read()
            filename = attachment_filename_override or os.path.basename(attachment_filepath_str)
            ...
            with gr.TabItem("Course Setup & Syllabus"):
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        class_days_selected = gr.CheckboxGroup(list(days_map.keys()), label="Class Days*", info="Select days...")
                    with gr.Column(scale=1):
                        gr.Markdown("#### Student & Access")
                        devices = gr.CheckboxGroup(["Phone","PC","Tablet"], label="Allowed Devices", value=["PC"])
                        students_input_str = gr.Textbox(label="Students (Name,Email per line)", lines=5,
                ...
                btn_save.click(
                    ...
                    outputs=[
                        output_box, save_error,
                        btn_save, dummy_btn_1,
                        dummy_btn_2, btn_edit_syl,
                        btn_email_syl, dummy_btn_3,
                        dummy_btn_4, syllabus_actions_row,
                        # ... remaining outputs up to 14
                    ],
                    queue=True
                )
